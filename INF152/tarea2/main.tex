...
\FOR{item in line}
    ...
    \STATE{\hskip1.0em list.append( stringToInt(line[x:y]) )}
    \STATE{\hskip1.0em x $\leftarrow$ y}
    \STATE{\textbf{end if}}
\ENDFOR
\STATE{\textbf{int} maxValue $\leftarrow$ 0}
\FOR{element1 in list}
    \STATE{\textbf{if} element1 $ >$ maxValue \textbf{do}}
    \STATE{\hskip1.0em maxValue $\leftarrow$ element1}
    \STATE{\textbf{end if}}
\ENDFOR
...
\FOR{element2 in list}
    \STATE{\textbf{if} element2 $ >$ givenNumber \textbf{and} element2 $ < $ maxValue \textbf{do}}
    \STATE{\hskip1.0em maxValue $\leftarrow$ element2}
    \STATE{\textbf{endif}}
\ENDFOR
...
\begin{enumerate}
    ...
    \begin{tikzpicture}
        \tikzstyle{level 1}=[sibling distance=4.3cm, level distance=1.9cm]
        \tikzstyle{level 2}=[sibling distance=1.9cm, level distance=2.4cm]
        ...
        \node {INF152}
            ...
            child {node {Amogus}
                child[grow= -120, level distance=2.75cm] {node {SUS}
                    child {node {Notas Tarea1}}
                    child {node {Notas Tarea2}}
                    child {node {Notas Tarea3}}}
                child[grow= -75, level distance=2.55cm] {node {Tripulante}
                    child {node {Jueguito}}
                    child {node {README}}}
        }
            child {node {Clases}
